...
<script>
const RANDOM_ICON_COUNT = 8;

export default {
  props: ["active", "toggle", "title", "subTitle", "icon"],
  computed: {
    iconValue: {
      get: function () {
        return this.icon;
  ...
  watch: {
    selected: {
      handler: function (newIndex) {
        ...
        this.$emit("update:icon", this.generatedIcons[newIndex]);
      },
      deep: true,
    },
  },
  mounted: function () {
    this.generateNewIcons();
    if (this.icon != null) {
      this.generatedIcons[this.selected] = this.icon;
    } else {
      this.generatedIcons[this.selected] = "mdi-web";
      this.$emit("update:icon", "mdi-web");
    }
  },
  methods: {
    generateNewIcons() {
      const currentIcon = this.generatedIcons[this.selected];
      this.generatedIcons = [];

      for (let index = 0; index < RANDOM_ICON_COUNT; index++) {
        const randomIcon = this.icons[this.randomIconPos()];
        this.generatedIcons.push(randomIcon);
      }
      this.selected = 0;
      this.generatedIcons[0] = currentIcon;
    },
    randomIconPos() {
    ...
    search() {
      ...
      for (let index = 0; index < this.icons.length; index++) {
        const iconRef = this.icons[index];
        const icon = iconRef.replace("-", " ").replace("mdi", "");

        if (icon.includes(this.searchValue)) {
          result.push(iconRef);

          if (result.length >= RANDOM_ICON_COUNT) {
            break;
          }
        }
      }
      this.generatedIcons = result;
    },
  },
  data: () => ({
    selected: 0,
